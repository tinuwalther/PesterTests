...
function Get-ServiceProperties{
    ...
    Write-verbose $function
    $ret = @()
    $wmiobj = Get-WmiObject Win32_Service -ErrorAction Stop | Where-Object Name -eq $ServiceName 
    if(-not([String]::IsNullOrEmpty($wmiobj))){
        $wmiobj | ForEach-Object{
            $obj = [PSCustomObject]@{
                Name        = $_.Name
...
function Get-RegistryProperty{
    ...
    Write-verbose $function
    $ret = $null
    $ret = Get-ItemProperty -Path $Path -Name $Property -ErrorAction SilentlyContinue | Select-Object -ExpandProperty $Property
    return $ret
}
